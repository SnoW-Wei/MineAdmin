<template>
  <div class="overlay" v-if="visible" @click.self="close">
    <div class="popup">
      ...
      <template v-else>
        <div>{{ $t('plugin.ljk123-captcha.slider.title') }}</div>
      </template>
      <template v-if="$slots.body">
        <slot name="body" :imageSrc="imageSrc" :imageWidthX="imageWidthX" />
      </template>
      <template v-else>
        <div class="image-body">
          <div class="image-container" v-if="imageSrc && sliderImageSrc">
            <img :src="imageSrc" class="slider-image-bg" ref="sliderImageBg" />
            <img :src="sliderImageSrc" class="slider-image-slider" ref="sliderImageSlider"
              :style="{ left: `${imageWidthX}%` }" />
          </div>
    ...
    $t('plugin.ljk123-captcha.slider.sliderTxt') }}</div>
        </div>
        <div class="slider" ref="slider" :style="positionLeftStyle">
          <div class="line"></div>
        </div>
        <div class="slider-container-text slidetounlock">{{ $t('plugin.ljk123-captcha.slider.sliderTxt') }}</div>
      </div>
      <div class="helper">
...
const sliderImageBg = ref(null)
const sliderImageSlider = ref(null)
const imageWidthX = computed(() => {
  //滑块的进度
  const slider = Math.max(0, Math.min((currentX.value / sliderWidth.value) * maxWidth, maxWidth));
  if (!sliderImageBg.value || !sliderImageBg.value) {
    return slider
  }
  const maxLeftScale = (sliderImageBg.value.offsetWidth - sliderImageSlider.value.offsetWidth) / sliderImageBg.value.offsetWidth
  return slider * maxLeftScale
})
...
const callGetCaptcha = () => {
  getCaptcha({
    ...
    imageSrc.value = data.option.bg;
    sliderImageSrc.value = data.option.slider;
    currentX.value = 0;
  });
};
...
.image-container .slider-image-bg {
  height: 100%;
  display: block;
  margin: 0 auto;
}

.image-container .slider-image-slider {
  position: absolute;
  left: 0;
  top: 0;
  height: 100%;
