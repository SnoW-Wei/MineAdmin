...
const captchaType = ref(isDevelop.value ? 0 : (parseInt(import.meta.env.VITE_APP_CAPTCHA_TYPE) || 0))
const handleSubmit = ({ values, errors }) => {
  captcha({ type: captchaType.value, feedbackUrl: "https://www.mineadmin.com/store/captcha" }).then(async (res) => {
    if (res.validated) {
      if (loading.value) {
        return
      }
    ...
    <div class="login-width md:w-10/12 w-11/12 mx-auto flex justify-between h-full items-center">
      ...
      <div class="md:w-6/12 w-11/12 md:rounded-r mx-auto pl-5 pr-5 pb-10">
        ...
        <a-form :model="form" @submit="handleSubmit">
          ...
          <a-form-item v-if="isDevelop" class="mt-5" :label="$t('plugin.ljk123-captcha.page.login.typeStyle')">
